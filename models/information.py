class OxygenInformation:
    ...
    def get_player_info(self, player_id, gauge_difficulty):
        ...
        registed_date = cursor.fetchone()[0]

        if registed_date is None:
            registed_date = "None"
        else:
            registed_date = registed_date.strftime("%Y-%m-%d, %H:%M:%S")
        ...
        return {
            ...
            "last_access_time": raw_player_info[5].strftime("%Y-%m-%d, %H:%M:%S")
            if raw_player_info[5] is not None
            else "None",
